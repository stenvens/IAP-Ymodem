...
bool Download::YmodemUploadFile(int waitFirstPacketAckMs)
{
    ...
    */
    ...
    //发送设备重启命令
    sendDeviceRebootPacket();
    qDebug("Send Device Reboot!\r\n");

    QThread::msleep(100);

//    //发送开始升级命令
//    sendStartUpdatePacket();
//    qDebug("Send Start Update!\r\n");
...
void Download::doWork1(const QString portName, const int baudRate, int fileSize, int yModemPackageSize)
{
    ...
    //Ymodem下载流程
    update_ret = YmodemUploadFile(10);
...
* @brief  : 发送<设备重启>命令
*********************************************************************/
void Download::sendDeviceRebootPacket()
{
    QByteArray ba;
    ba.resize(7);
    ba[0] = 0x5A;
...
* @brief  : 发送<开始升级>命令
*********************************************************************/
void Download::sendStartUpdatePacket()
{
    QString cmd("st");
