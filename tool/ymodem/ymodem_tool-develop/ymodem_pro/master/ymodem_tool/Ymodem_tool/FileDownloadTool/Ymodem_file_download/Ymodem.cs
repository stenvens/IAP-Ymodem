        ...
        */
        public void YmodemUploadFile(YModemType yModemType, int waitFirstPacketAckMs)
        {
            ...
            try
            {
                //发送设备重启命令
                SendDeviceReboot();
                Debugging.output("Send Device Reboot!");

                Thread.Sleep(100);

                //发送开始升级命令
                SendStartUpdate();
                Debugging.output("Send Start Update!");
        ...
        private void SendDeviceReboot()
        {
            byte[] packet = System.Text.Encoding.Default.GetBytes("reboot");
            serialPort.Write(packet, 0, packet.Length);
        }

        private void SendStartUpdate()
        {
            byte[] packet = System.Text.Encoding.Default.GetBytes("st");
            serialPort.Write(packet, 0, packet.Length);
        }
    }
}