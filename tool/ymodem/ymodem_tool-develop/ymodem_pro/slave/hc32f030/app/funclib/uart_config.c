...
void get_version_uart_callback(uint8_t *data, uint16_t size);
...
static struct debug_cmd_t uart_cmd_table[] = 
{
	...
	{"get_version",			get_version_uart_callback},
};
...
{
	char buf[50] = {0};
	sprintf(buf, "get_version=%s;", SOFTWARE_VERSION);
	uart_config_send(buf, strlen(buf));
}
...
static void clear_uart_data(void)
{
	...
	memset(uart1_rx_buf, 0x00, UART1_BUF_MAXIMUM);
	uart1_rx_cnt = 0;
}
...
void uart_config_send(char *buf, uint16_t length)
{
	...
	bsp_uart1_send(config_uart.send_buf, config_uart.send_length);
}
